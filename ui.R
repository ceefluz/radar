...
library(shinyalert)
    ...
    ui <- dashboardPage(
      ...
      dashboardHeader(
        ...
        titleWidth = 300,
        dropdownMenu(
          type = "notifications", headerText = strong("HELP"), icon = icon("question"), badgeStatus = NULL,
          notificationItem(
            text = (steps$text[1]),
            icon = icon("spinner")
          ),
        ...
        tags$li(
          a(
            strong("ABOUT RadaR"),
            height = 40,
            href = "https://github.com/ceefluz/radar/",
            title = "",
            target = "_blank"
          ),
          class = "dropdown"
                 ...
                 sidebarMenu(
                   introBox(data.step = 1, data.intro = intro$text[1],
                            div(id = "sidebar_button",
                                bsButton(inputId = "confirm", label = "START RADAR", icon = icon("play-circle"), style = "danger")
                            )
                   ),
                   div(class = "inlay", style = "height:15px;width:100%;background-color: #ecf0f5;"),
                   conditionalPanel(
                     ...
                     sidebarMenu(
                       menuItem(
                         ...
                         selectInput(
                           ...
                           label = "",
                           selected = "Antimicrobial consumption",
                           multiple = FALSE
                         )
                         ,
                         br(),
                   ...
                   menuItem(
                     ...
                     menuItem(
                       ...
                       switchInput(
                         ...
                         label = "SELECTION",
                         value = TRUE, onLabel = "SLIDER", offLabel = "ANY", inline = TRUE, size = "mini", width = "100%", offStatus = "danger"
                       )
                     ),
                     menuItem(
                       ...
                       sliderInput(
                         ...
                         value = 2L,
                         min = 1L,
                         max = max(set$ab_days_use, na.rm = TRUE),
                         step = 1L
                       ),
                       switchInput(
                     ...
                     menuItem(
                       ...
                       checkboxGroupInput(
                         inputId = "adminInput",
                         label = "",
                         choiceNames = c("IV", "ORAL"),
                         choiceValues = c("IV", "ORAL"),
                         selected = "IV",
                         inline = TRUE
                       )
                     ),
                     menuItem(
                       ...
                       switchInput(
                         inputId = "firstInput",
                         label = "",
                         value = TRUE, onLabel = "FIRST", offLabel = "ALL", size = "mini", width = "100%", offStatus = "danger"
                       )
                     ),
                     ...
                     checkboxGroupInput(
                       ...
                       label = "",
                       choices = sort(ab_group$ab_group)
                     ),
                     checkboxGroupInput(
                       inputId = "abInput",
                   ...
                   menuItem(
                     ...
                     sliderInput(
                       inputId = "ageInput",
                       label = "Age",
                       value = c(min(set$age), max(set$age)),
                       min = min(set$age),
                       max = max(set$age),
                       step = 1,
                       sep = ""
                   ...
                   menuItem(
                     ...
                     selectizeInput(
                       ...
                       choices = c(levels(as.factor(set$specialty_detail))),
                       multiple = TRUE
                     ),
                     selectizeInput(
                       inputId = "exInput",
                   ...
                   menuItem(
                     ...
                     checkboxGroupInput(
                       ...
                       label = "",
                       choices = levels(as.factor(set$adm_route_sum)),
                       selected = levels(as.factor(set$adm_route_sum))
                     )
                   ),
                   br(),
                   br(),
                   menuItem(
                     ...
                     selectInput(
                       ...
                       label = "",
                       choices = list("Blood cultures" = "bc_check", "Urine cultures" = "uc_check")
                     ),
                     sliderInput(
                       inputId = "checkInput",
                   ...
                   menuItem(
                     ...
                     textInput(
                       inputId = "filename",
                       placeholder = "SORRY! No download in demo mode.",
                       label = ""
                     ) 
                   ),
                   br()
      ...
      dashboardBody(
        tags$head(
          tags$link(rel = "stylesheet", type = "text/css", href = "radar_style.css"),
        ...
        fluidRow(
          column(
            width = 12,
            introBox(
              bsButton("patients", label = "PATIENTS", icon = icon("user"), style = "success"),
              bsButton("antimicrobials", label = "ANTIMICROBIALS", icon = icon("spinner", class = "spinner-box"), style = "success"),
              bsButton("diagnostics", label = "DIAGNOSTICS", icon = icon("flask", class = "flask-box"), style = "success"),
              bsButton("outcome", label = "OUTCOME", icon = icon("thumbs-o-up"), style = "success"),
